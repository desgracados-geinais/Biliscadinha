...
function playerLogin(username,password,checksave)
	...
	if exports.bans:checkAccountBan(accountData["id"]) then
		triggerClientEvent(client,"set_warning_text",client,"Login","Account is banned. Appeal at trinitys homepage")
		return false
	end
	...
	for _, thePlayer in ipairs(exports.pool:getPoolElementsByType("player")) do
		...
		if (playerAccountID) then
			if (playerAccountID == tonumber(accountData["id"])) and (thePlayer ~= client) then
				...
				triggerClientEvent(thePlayer,"set_authen_text",thePlayer,"Login","Account is currently online. Disconnecting other user..")
				break
			end
		end
	end
	...
	--fetchRemote ( "----" .. tonumber(accountData["id"]) .. "&type=thumb", myCallback, "", false, accountData["id"] )
	--[[
	--ADMINS
	...
	if tonumber(accountData["appstate"]) < 3 then
		if exports.integration:isPlayerTrialAdmin(client) or exports.integration:isPlayerSupporter(client) then
			...
			triggerClientEvent(client,"account:showRules",client, tonumber(accountData["appstate"]) )
			return false
		end
	end
...
function playerRegister(username,password,confirmPassword, email)
	...
	preparedQuery3 = "INSERT INTO `accounts` SET `username`='"..toSQL(username).."', `password`='"..toSQL(encryptedPW).."', `email`='"..toSQL(email).."', `registerdate`=NOW(), `ip`='"..toSQL(ipAddress).."', `salt`='"..toSQL(encryptionRule).."', `mtaserial`='"..mtaSerial.."', `activated`='0' "
