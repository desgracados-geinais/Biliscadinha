...
function adminhelp (commands, scriptUpdates)
	...
	if not exports.integration:isPlayerSeniorAdmin(getLocalPlayer()) and not getElementData(getLocalPlayer(), "hasCmdLibraryAccess") then
	...
	if exports.integration:isPlayerSeniorAdmin(getLocalPlayer()) or getElementData(getLocalPlayer(), "hasCmdLibraryAccess") then
		...
		if selectedTab ~= nil then 
			for i = 1, #tab do
				if isElement(tabPanel) and guiGetSelectedTab ( tabPanel ) == tab[i] then
					setElementData(getLocalPlayer(), "cmd:currentTabIndex", i)
					break
				end
			end
		end	
...
function copyToClipboard1(button,state)
	...
	if (row==-1) or (col==-1) then
		...
		elseif (exports.integration:isPlayerSeniorAdmin(getLocalPlayer()) or getElementData(getLocalPlayer(), "hasCmdLibraryAccess")) and (button == "right") and (state == "up") then
